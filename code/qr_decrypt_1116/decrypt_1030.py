...
def read_eeprom_length(address, mem_address, max_length):
    length = 0  # 初始化長度計數
    for i in range(mem_address, mem_address + max_length):
        try:
            # 逐位元組讀取資料
            byte = bus.read_byte_data(address, i)
            length += 1  # 每次成功讀取，長度加 1

            # 如果遇到符號（如 }），停止計算（可選）
            if byte ==ord('}'):  
                print("Detected end of data.")
                break
        except Exception as e:
...
def main():
    ...
    data_length = read_eeprom_length(DEVICE_ADDRESS, START_MEMORY_ADDRESS,MAX_BYTES)

    # 讀取 EEPROM 中的數據
    eeprom_data = read_eeprom(DEVICE_ADDRESS, START_MEMORY_ADDRESS, data_length)
